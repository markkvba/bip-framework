...
public class BipFeignRuntimeExceptionTest {
	...
	public void instantiateBipFeignRuntimeExceptionTest() throws Exception {
		...
		Assert.assertTrue(bipFeignRuntimeException.getExceptionInfo().getKey().equals(TEST_KEY.getKey()));
		Assert.assertTrue(bipFeignRuntimeException.getMessage().equals(TEST_TEXT));
		Assert.assertTrue(bipFeignRuntimeException.getExceptionInfo().getStatus().equals(TEST_HTTP_STATUS));
		Assert.assertTrue(bipFeignRuntimeException.getExceptionInfo().getSeverity().equals(TEST_SEVERITY));
	}
