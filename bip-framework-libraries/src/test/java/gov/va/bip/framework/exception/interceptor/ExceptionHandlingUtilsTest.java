...
public class ExceptionHandlingUtilsTest {
	...
	public void testConvertFromBipExceptionExtender() {
		...
		assertTrue(resolvedRuntimeException.getExceptionInfo().getKey().equals(TEST_KEY.getKey()));
		assertTrue(resolvedRuntimeException.getMessage().equals(TEST_KEY_MESSAGE));
		assertTrue(resolvedRuntimeException.getExceptionInfo().getSeverity().equals(MessageSeverity.ERROR));
		assertTrue(resolvedRuntimeException.getExceptionInfo().getStatus().equals(HttpStatus.BAD_REQUEST));
	}
	...
	public void testConvertFromBipExceptionExtenderWithClassCastException() {
		try {
			...
			assertTrue(e.getExceptionInfo().getKey().equals(MessageKeys.BIP_EXCEPTION_HANDLER_ERROR_VALUES.getKey()));
			assertTrue(e.getMessage().equals(MessageKeys.BIP_EXCEPTION_HANDLER_ERROR_VALUES.getMessage((String[]) null)));
			assertTrue(e.getExceptionInfo().getSeverity().equals(MessageSeverity.FATAL));
			assertTrue(e.getExceptionInfo().getStatus().equals(HttpStatus.INTERNAL_SERVER_ERROR));
		}
	}
	...
	public void testCastToBipRuntimeException() {
		try {
			...
			assertTrue(e.getExceptionInfo().getKey().equals(""));
	...
	public void testCastToBipRuntimeExceptionCatchBlock() {
		try {
			...
			assertTrue(e.getExceptionInfo().getMessageKey().equals(MessageKeys.BIP_EXCEPTION_HANDLER_ERROR_CAST));
